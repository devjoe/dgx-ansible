#!/usr/bin/env python3
"""Evaluate Ollama local/cloud models on DS4 settled/contested prompts."""
...
def post_chat(
    ...
    except Exception as exc:  # noqa: BLE001 - runner preserves failures.
        elapsed = time.perf_counter() - started
...
def classify(item: dict[str, Any], answer: str) -> dict[str, Any]:
    ...
    return {
        ...
        "compatible_topic_contestedness": topic in expected,
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for category in sorted({row["category"] for row in rows}):
        ...
        by_category[category] = {
            ...
            "topic_compatible": sum(1 for row in scoped if cls(row).get("compatible_topic_contestedness")),
            ...
            "empty_answer": sum(1 for row in scoped if row.get("http_ok") and not row.get("answer")),
        }
    return {
        "n": len(rows),
        "http_ok": sum(1 for row in rows if row.get("http_ok")),
        "topic_compatible": sum(1 for row in rows if cls(row).get("compatible_topic_contestedness")),
...
def render_html(report: dict[str, Any]) -> str:
    ...
    for row in report["results"]:
        issue_text = ", ".join(cls(row).get("issues") or []) or "none"
        cards.append(
            f"""
            <section class="case">
              <div class="meta">
                <strong>{html.escape(str(row['id']))}</strong>
                <span>{html.escape(str(row['category']))}</span>
                <span>topic={html.escape(str(cls(row).get('topic_contestedness', '')))}</span>
                <span>issues={html.escape(issue_text)}</span>
                <span>answer_chars={len(str(row.get('answer') or ''))}</span>
              </div>
              <h3>{html.escape(str(row.get('topic') or ''))}</h3>
              <pre>{html.escape(str(row.get('answer') or row.get('reasoning') or row.get('error') or ''))}</pre>
            </section>
            """
        )
...
<head>
  ...
  <title>Ollama DS4 Settled/Contested Eval</title>
  <style>
...
<body>
<main>
  <h1>Ollama Cloud/Local: DS4 Settled/Contested Eval</h1>
  <div class="sub">Model: {html.escape(report['model'])} · mode={html.escape(report['mode'])} · generated={html.escape(report['generated_at'])}</div>
  <section class="grid">
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("reports/ollama-ds4-contestedness"))
    ...
    print(f"Running {len(items)} DS4 cases against Ollama model {args.model}", flush=True)
    for index, item in enumerate(items, start=1):
        prompt = question_for(item, args.mode)
        status, payload, error, elapsed = post_chat(
            args.endpoint,
            args.model,
            prompt,
            args.mode,
        ...
        print(
            ...
            f"topic={cls(row).get('topic_contestedness', 'ERR')} "
            f"issues={','.join(cls(row).get('issues', [])) or '-'}",
            flush=True,
        )
    ...
    base = f"{stamp}-{slug(args.model)}-{args.mode}"
